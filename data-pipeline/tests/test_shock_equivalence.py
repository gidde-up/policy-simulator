...
def test_compiled_channels_match_v1_builders(engine, iso3):
    ...
    channels = engine._scenario_channels(
        ...
        include_retaliation=True, include_financing_drag=True)
    ...
    expected = {
        ...
        "financing_drag": engine.financing_drag_dF(
            cd, sum(r * cd.x[cd.sectors.index(s)]
...
def test_financing_drag_only_from_support(engine, iso3):
    cd = engine.load_country(iso3)
    p = engine.load_params("central", iso3)
    # stimulus only -> no financing_drag channel
...
def test_import_price_shock_matches_price_effects(engine, iso3):
    ...
    channels, _ = engine._evaluate_channel_dFs(cd, p, shocks,
                                               include_financing_drag=True)
